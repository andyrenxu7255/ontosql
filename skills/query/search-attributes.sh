...
sql="SELECT json_agg(row_to_json(t)) FROM (
    SELECT attr_name, attr_id, COALESCE(description,'') as description,
    ...
    FROM ontosql.search_attributes(
        '${query_text//\'/\'\'}',
        '${graph_name}',
        ${top_k},
        ${embedding_param}
...
result=$("${PSQL_CMD[@]}" -c "${sql}" 2>&1) || die_server "ERR_QUERY_FAILED" "search_attributes query failed" "{\"sql_error\":\"$(echo "${result}" | head -1 | sed 's/"/\\"/g')\"}"
...
output_success "search-attributes" "${ELAPSED}" '{"results":'"${data}"',"count":'"${rows}"'}'